...
public class DBUtils {
	...
	public static List<Answer> getPossibleAnswers(int[] topics, int maxLength) {
		...
		String sqlQuery = "select answers.id, answer, answers.entity_id as entity_id, additional_information "
				+ "from answers, entities , entities_definitions, definitions, definitions_topics " + "where "
				+ "answers.entity_id = entities.id and " + "entities.id = entities_definitions.entity_id and "
				+ "entities_definitions.definition_id = definitions_topics.definition_id and " + topicsCondition
				+ " and " + maxLenghtCondition + ";";
	...
	private static Map<Integer, List<String>> getHints(String entityIds) {
		String sqlHintsQuery = "select entity_id, yago_hint, is_entity_subject, subject_str, object_str "
				+ "from hints h, predicates p " + "where h.predicate_id = p.id and " + "entity_id in " + entityIds
				+ ";";
	...
	private static Map<Integer, List<String>> getDefinitions(String entityIds) {
		String sqlDefinitionsQuery = "select entities.id as entity_id, definitions.definition as definition "
				...
				+ "entities_definitions.definition_id = definitions.id and " + "entity_id in " + entityIds + ";";
	...
	public static Map<String, Integer> getDefinitionsByEntityID(int entityID) {
		String sql = "SELECT definitions.id, definitions.definition "
				...
				+ "WHERE entities.id = entities_definitions.entity_id AND "
				+ "definitions.id = entities_definitions.definition_id " + "AND entities_definitions.entity_id IN "
	...
	public static Map<String, Integer> getAllDefinitions() {
		String sql = "SELECT definitions.id,definitions.definition " + "FROM definitions";
	...
	public static Map<String, Integer> getTopicByDefinitionIDs(List<Integer> definitionIDs) {
		String sql = "SELECT topics.id, topics.name " + "FROM topics, definitions_topics, definitions"
				+ "WHERE (topics.id = definitions_topics.topic_id) "
				+ "AND (definitions_topics.definition_id = definitions.definition) " + "AND (definitions.id IN "
				+ createINString(definitionIDs) + ");";
	...
	public static String[][] getTenBestScores() {
		String sql = "SELECT best_scores.user_name, best_scores.score " + "FROM best_scores "
				+ "ORDER BY best_scores.score DESC " + "LIMIT 10;";

		List<Map<String, Object>> lst = DBConnection.executeQuery(sql);
		String[][] returnArray = new String[lst.size()][2]; // each of the 10
															// cells is a tuple
															// [name,score]
	...
	public static String[] getTriviaQuestion() {
		...
		sqlQuery = "SELECT a.answer, d.definition, a.additional_information "
				...
				+ "WHERE 	e.id = ed.entity_id AND " + "ed.definition_id = d.id AND " + "e.id = a.entity_id AND "
				+ "e.id = " + randEntity + " " + "ORDER BY a.length " + "LIMIT 1; ";
	...
	public static void deleteEntityDefinition(int entity_id, int definition_id) {
		String sqlQuery = "DELETE FROM entities_definition WHERE entity_id = " + entity_id + " AND definition_id = "
				+ definition_id + ";";
