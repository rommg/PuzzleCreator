		...
		for (PuzzleDefinition def : view.getDefinitions()) { // iterate each definition to check if the letters in its domain are correct
			...
			case 'r': {
				for (int col = def.getBeginColumn(); col<def.getBeginColumn() + def.getAnswer().length; col++) {
					...
					char letter = ((SquareTextField)boardPanelHolders[def.getBeginRow()][col].getComponent(0)).getText().trim().charAt(0); // there should be only one letter 
			...
			case 'l':{
				for (int col = def.getBeginColumn(); col>def.getBeginColumn() - def.getAnswer().length; col--) {
					char letter = ((SquareTextField)boardPanelHolders[def.getBeginRow()][col].getComponent(0)).getText().trim().charAt(0); 
					result &= (letter == answer[indexInAnswer]);
				}
				break;
			}
			case 'u': {
				for (int row = def.getBeginRow(); row>def.getBeginRow() - def.getAnswer().length; row--) {
					char letter = ((SquareTextField)boardPanelHolders[row][def.getBeginColumn()].getComponent(0)).getText().trim().charAt(0); 
					result &= (letter == answer[indexInAnswer]);
				}
				break;
			}
